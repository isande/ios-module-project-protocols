...
struct Deck {
    ...
    init(ranks: [CardValue], suits: [CardSuit]) {
        var cards: [Card] = []
...
class HighLow: CardGame {
    ...
    func play() {
        ...
        if playerOneCard == playerTwoCard {
            ...
            print("Player 1 wins with \(playerOneCard.rank.description) of \(playerOneCard.suit.rawValue)")
        } else if playerTwoCard > playerOneCard {
            print("Player 2 wins with \(playerTwoCard.rank.description) of \(playerTwoCard.suit.rawValue)")
        } else {
...
// Updated Step 14 from readme:
// As part of the protocol conformance, implement a method called play().
// The method should draw 2 cards from the deck, one for player 1 and one
// for player 2. These cards will then be compared to see which one is
// higher. The winning player will be printed along with a description of
// the winning card. Work will need to be done to the Suit and Rank types
// above, so see the next couple steps before continuing with this step.
...
// Updated Step 15 from readme:
// Take a look at the Swift docs for the Comparable protocol.
// In particular, look at the two functions called < and ==.
...
// Updated Step 16 from Readme:
// Make the Rank type conform to the Comparable protocol. Implement the <
// and == methods such that they compare the rawValue of the lhs and rhs
// arguments passed in. This will allow us to compare two rank values with
// each other and determine whether they are equal, or if not, which one is
// larger.
...
// Updated Step 17 from Readme:
// Make the Card type conform to the Comparable protocol. Implement the <
// and == methods such that they compare the ranks of the lhs and rhs
// arguments passed in. For the == method, compare both the rank and the
// suit.
...
//: * Player 1 wins with a higher card, e.g. "Player 1 wins with 8 of hearts."
//: * Player 2 wins with a higher card, e.g. "Player 2 wins with king of diamonds."
// Updated Step 19 from Readme:
// Time to test all the types you've created. Create an instance of the
// HighLow class. Lastly, call the play() method on the game object.

let myDeck = Deck(ranks: CardValue.allRanks, suits: CardSuit.allSuits)
let myGame = HighLow(deck: myDeck)
